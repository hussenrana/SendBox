...
public class Cell {
	...
	//this method fill the cell of path
	...
	public void draw(Graphics g) {
	    ...
	    if (visited) {
	    	g.setColor(Color.decode("#2276D0"));
	...
	//this method for each cell which have 4 walls, we deleted the suitable wall according to the situation 
	public void removeWalls(Cell next) {
		int x = this.x - next.x;
		 // top 0, right 1, bottom 2, left 3
		
		if(x == 1) {
			KruskalsGen.currentlog+=". we remove left wall from "+this.getX()+","+this.getY()+" and right wall from "+next.getX()+","+next.getY();
			Maze.log.append("We remove left wall from "+this.getX()+","+this.getY()+" and right wall from "+next.getX()+","+next.getY()+"\n");
			walls[3] = false;
			next.walls[1] = false;
		} else if (x == -1) {
			KruskalsGen.currentlog+=". we remove right wall from "+this.getX()+","+this.getY()+" and left wall from "+next.getX()+","+next.getY();
			Maze.log.append("We remove right wall from "+this.getX()+","+this.getY()+" and left wall from "+next.getX()+","+next.getY()+"\n");
			walls[1] = false;
			next.walls[3] = false;
		}
		...
		if(y == 1) {
			KruskalsGen.currentlog+=". we remove top wall from "+this.getX()+","+this.getY()+" and buttom wall from "+next.getX()+","+next.getY();
			Maze.log.append("We remove top wall from "+this.getX()+","+this.getY()+" and buttom wall from "+next.getX()+","+next.getY()+"\n");
			walls[0] = false;
			next.walls[2] = false;
		} else if (y == -1) {
			KruskalsGen.currentlog+=". we remove buttom wall from "+this.getX()+","+this.getY()+"and top wall from "+next.getX()+","+next.getY();
			Maze.log.append("We remove buttom wall from "+this.getX()+","+this.getY()+"and top wall from "+next.getX()+","+next.getY()+"\n");
			walls[2] = false;
			next.walls[0] = false;
		}
	}
//choose the neighbour randomly
	...
	// check if the cell is Neighbour
	...
	//check if Neighbour is exist in list
	...
	// return all Neighbour for specific cell
